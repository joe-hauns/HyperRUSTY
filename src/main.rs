#![allow(warnings)]
use clap::{ArgGroup, Command, arg, value_parser};
use enchelper::*;
use encoder::*;
use hqb_verilog::*;
use ir::*;
...
use unroller_qbf::*;
use z3::{
    Config, Context, SatResult, Solver, StatisticsValue,
...
fn main() {
    ...
    let trajectory_bound = matches.get_one::<usize>("trajectory_bound");

    let formula_path = matches.get_one::<PathBuf>("formula").unwrap();
    ...
    if let Some(nusmv_models) = matches.get_many::<PathBuf>("nusmv") {
        ...
        if *matches.get_one::<bool>("qbf_solver").unwrap() {
            ...
            if model_paths.len() != path_identifiers.len() {
                panic!(
                    "ERROR: number of provided models and number of path quantifiers do not match!"
                );
            }
            ...
            let out_path = "_outfiles/quabs.out";
            {
                ...
                file.write_all(&output.stdout)
                    .expect("Failed to write QuAbs output");
            }
        ...
        for i in 0..path_identifiers.len() {
            // parse the smv for this model
            let env =
                build_smvenv_from_verilog(build_paths[i], top_module, yosys_out, &ctx).unwrap();
            envs.push(env);
        }
        let duration = start.elapsed();
        let secs = duration.as_secs_f64();
